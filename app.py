...
HELP = {
    ...
    "score": "ציון איכות 0-6 מה-PDF: צמיחת מכירות/רווח, שוליים, ROE, ומזומן מול חוב.",
    "pl": "הרווח או ההפסד הכספי שלך על הנייר.",
    "yield": "השינוי באחוזים ממחיר הקנייה המקורי שלך.",
    "action": "המלצת AI לפעולה מיידית.",
    "logic": "ניתוח מפורט של ה-AI המשלב את איכות החברה מול מחיר השוק.",
...
def get_ai_rec_detailed(price, fv, score):
    if not fv or fv == 0: return "בבדיקה 🔍", "אין מספיק נתונים לחישוב שווי הוגן."
    gap = (fv - price) / price
    if score >= 5:
        if gap > 0.05: return "קנייה חזקה 💎", f"מניית זהב (ציון {score}). נסחרת בנחה של {abs(gap):.1%} מהשווי שלה."
        return "קנייה 📈", "חברה מעולה במחיר הוגן. פוטנציאל צמיחה גבוה."
    elif score >= 3:
        if gap > 0.10: return "איסוף 🛒", "חברה טובה שנמצאת כרגע במחיר מבצע יחסית לרווחים."
        return "החזק ⚖️", "החברה יציבה אך המחיר משקף את השווי האמיתי שלה כרגע."
    return "מכירה/המתנה 🔴", "ציון איכות נמוך יחסית לסיכון בשוק כרגע."
...
def fetch_hub_data(tickers):
    ...
    for t in tickers:
        try:
            ...
            action, logic = get_ai_rec_detailed(px, fv, score)
...
tabs = st.tabs(["📌 המניות שלי", "🔍 סורק מניות זהב", "💰 דיבידנדים", "📄 אודות ו-10 שנים", "🤝 רדאר מיזוגים"])
...
# טאב 2: סורק
with tabs[1]:
    st.subheader("🔍 סריקת הזדמנויות לפי המדריך")
...
with tabs[2]:
    ...
    st.dataframe(div_df[["Symbol", "Dividend", "ExDateClean"]], column_config={"Dividend": st.column_config.NumberColumn("דיבידנד %", format="%.2f%%")}, use_container_width=True, hide_index=True)
...
with tabs[3]:
    ...
    with col_bear: st.markdown(f'<div class="bear-box"><b>🐻 דוב:</b> תמחור מתוח בטווח הקצר.</div>', unsafe_allow_html=True)
    ...
    fig.update_layout(title=f"ביצועי {sel} ל-{yrs} שנים", height=350, template="plotly_white")
...
with tabs[4]:
    ...
    for m in mergers:
        st.markdown(f'<div class="ai-card"><b>{m["חברה"]}</b> | סבירות AI: {m["סבירות"]}<br><a href="{m["לינק"]}" target="_blank">🔗 לכתבות האחרונות</a></div>', unsafe_allow_html=True)
